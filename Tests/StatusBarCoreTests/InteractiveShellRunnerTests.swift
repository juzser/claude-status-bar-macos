...
@Suite("InteractiveShellRunner")
struct InteractiveShellRunnerTests {
    ...
    func resolvesPathViaInteractiveShellRC() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        ...
        let launcher = dir.appendingPathComponent("fake-launcher")
        ...
        let repairedResult = await InteractiveShellRunner.invoke(
        ...
        let unrepairedResult = await InteractiveShellRunner.invoke(
            binary: launcher.path, arguments: [], timeout: 5,
            environment: [
                "HOME": emptyHome.path,
                "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
    ...
    func quotesArgumentsWithSpaces() async throws {
        ...
        let result = await InteractiveShellRunner.invoke(
    ...
    func diagnosticLogCapturesOutput() async throws {
        ...
        let secondResult = await InteractiveShellRunner.invoke(
    ...
    @Test("captureOutput returns stdout trimmed of surrounding whitespace and newlines")
    func captureOutputReturnsTrimmedStdout() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        let script = dir.appendingPathComponent("echo-value.sh")
        try "#!/bin/sh\nprintf '  \\n hello-value \\n\\n'\nexit 0\n"
            .write(to: script, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o755],
                                              ofItemAtPath: script.path)

        let result = await InteractiveShellRunner.captureOutput(
            binary: script.path, arguments: [], timeout: 5,
            environment: ["HOME": dir.path, "PATH": "/usr/bin:/bin:/usr/sbin:/sbin"])
        #expect(result == "hello-value")
    }

    @Test("captureOutput returns nil on nonzero exit even when stdout has output")
    func captureOutputReturnsNilOnNonzeroExit() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        let script = dir.appendingPathComponent("fail-with-output.sh")
    ...
    @Test("captureOutput returns nil when stdout is empty on exit 0")
    func captureOutputReturnsNilOnEmptyStdout() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        let script = dir.appendingPathComponent("silent.sh")
        try "#!/bin/sh\nexit 0\n".write(to: script, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o755],
                                              ofItemAtPath: script.path)
    ...
    @Test("captureOutput strips shell startup banner text and OSC escape sequences")
    func captureOutputStripsShellStartupNoise() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        // Reproduces Apple Terminal's /etc/zshrc session-restore integration,
        // which (when TERM_SESSION_ID is unset — true for any GUI-app-spawned
        // subprocess) prints a banner line and an OSC7 "current directory"
        // escape sequence directly adjacent to the real command's stdout,
        // with no separating newline before it.
        let zdotdir = dir.appendingPathComponent("zdotdir")
        try FileManager.default.createDirectory(at: zdotdir, withIntermediateDirectories: true)
        try """
        echo 'Restored session: Fri Jul 17 13:46:47 +07 2026'
        printf '\\033]7;file://Ser-Mac.local/Users/ser\\007'
        """.write(to: zdotdir.appendingPathComponent(".zshrc"), atomically: true, encoding: .utf8)

        let script = dir.appendingPathComponent("echo-path.sh")
        try "#!/bin/sh\nprintf '/Users/ser/.local/bin/claude\\n'\n"
            .write(to: script, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o755],
                                              ofItemAtPath: script.path)
        ...
        #expect(result == "/Users/ser/.local/bin/claude")
    }

    @Test("captureOutput writes diagnosticLog with stdout, stderr, and exit code")
    func captureOutputWritesDiagnosticLog() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("interactive-shell-runner-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        let script = dir.appendingPathComponent("noisy-capture.sh")
        try "#!/bin/sh\necho out-marker\necho err-marker 1>&2\nexit 0\n"
            .write(to: script, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o755],
                                              ofItemAtPath: script.path)
        ...
        let result = await InteractiveShellRunner.captureOutput(
            ...
            environment: ["HOME": dir.path, "PATH": "/usr/bin:/bin:/usr/sbin:/sbin"],
            diagnosticLog: diagnosticLog)
        #expect(result == "out-marker")

        let contents = try String(contentsOf: diagnosticLog, encoding: .utf8)
        #expect(contents.contains("out-marker"))
        #expect(contents.contains("err-marker"))
        #expect(contents.contains("exitCode: 0"))
    }
}
